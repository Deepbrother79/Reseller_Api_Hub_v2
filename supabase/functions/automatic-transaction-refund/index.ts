...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (configError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch refund configurations' }),
        { 
          status: 500,
    ...
    for (const config of refundConfigs || []) {
      ...
      const payloads = config.payloads.split(',').map((p: string) => p.trim().toLowerCase())
      ...
      const eligibleTransactions = transactions?.filter((transaction: Transaction) => {
      ...
      for (const transaction of eligibleTransactions) {
        try {
          // Update transaction output_result to "Order Refunded"
          const { error: updateError } = await supabaseClient
            .from('transactions')
            .update({ output_result: ["Order Refunded"] })
          ...
          const currentNote = tokenData.Note || ''
          const refundNote = `${currentNote} Refunded (${transaction.id})`.trim()
